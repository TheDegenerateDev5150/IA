#!/bin/sh
# (r)  Antonio Taboada - hackingyseguridad.com 2026

# integracion de modelos de ollama para poder usarlose en opencode
# los modelos que tengamos descargados e instalados en ollama , los podemos ver, selecioar y usar en opencode
# por ejemplo si hemos instalado previamente:  ollama run deepseek-r1:1.5b
# podremos selecionar y ejecutar con opencode  -> deepseek-r1:1.5b, ideal para trabjar en modo local, sin internet!!! privacidad 
# ollama launch opencode --model deepseek-r1:1.5b

ollama launch opencode

